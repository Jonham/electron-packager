...
代码复制到临时目录后，这些函数会被执行。每个函数都会接收到一下5个参数。
...
如果函数需要串行调用，请使用serialHooks。但需要注意
**`serialHooks`不支持使用回调函数.** 例如:
...
**注意:** 如果设置了 [`prebuiltAsar`](#prebuiltasar)，`afterCopy`就不会被调用。
...
[Electron发布版]: https://github.com/electron/electron/releases
